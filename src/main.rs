...
Use `thought-castle inventory .` before extracting notes so every normalized session is visible.

When running archive intake with an agent, ask it to report created notes, skipped items with reasons, deferred sessions, and anything that still needs human verification.
"#;
